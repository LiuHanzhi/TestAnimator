...
//        setHasFixedSize(true);
    ...
    public boolean requestFocus(int direction, Rect previouslyFocusedRect) {
        ...
        if (null == getFocusedChild()) {
            Log.e("lhz", "子view没有焦点，寻找焦点");
            //请求默认焦点
            requestDefaultFocus(direction);
        } else {
            Log.e("lhz", "子view有焦点，停止寻找焦点");
        }
        return true;
//        return false;
    }
...
//        }
    ...
    private void requestDefaultFocus(int direction) {
        ...
        if (mSelectedPosition < 0) {
            Log.e("lhz", "没有寻找到可被focus的焦点，从子view中寻找第一个可被focus的焦点");
            for (int i = 0; i < getChildCount(); i++) {
                View view = getChildAt(i);
                if (view.isFocusable()) {
                    mSelectedPosition = i;
        ...
        if (mSelectedPosition < 0) {
            Log.e("lhz", "子view中没有可被focus的焦点，返回");
            return;
        }
